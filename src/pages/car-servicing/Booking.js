...
function CarServicing() {
  const [showCarServicingTabs, setshowCarServicingTabs] = useState(1);
  return (
    <>
      <div
      ...
      >
        <div className="container">
          ...
          <div className="tab-content">
            {showCarServicingTabs == 1 && <PersonalDetails />}
            {showCarServicingTabs == 2 && <VehicleDetails />}
            {showCarServicingTabs == 3 && <ServiceDetails />}
            {showCarServicingTabs == 4 && <BookingSummary />}
          </div>
          ...
          >
            <button
              className={
                showCarServicingTabs === 1
                  ? "btn sw-btn-prev disabled"
                  : "sw-btn-prev custom-btn theme-2"
              }
              type="button"
              onClick={() => {
                if (showCarServicingTabs > 1) {
                  setshowCarServicingTabs(showCarServicingTabs - 1);
                } else {
                  return;
                }
              }}
            ...
            {showCarServicingTabs <= 3 && (
              <button
                onClick={() => {
                  if (showCarServicingTabs <= 3) {
                    setshowCarServicingTabs(showCarServicingTabs + 1);
                  } else {
                    return;
                  }
            ...
            {showCarServicingTabs == 4 && (
              <button>
                <Link
                  to="/PayNow"
                  className="btn-info custom-btn theme-2 service_submit_btn"
                >
                  Submit
                </Link>
              </button>
